...
init_ui_manager()
{
	...
	menus_action_group->add( Gtk::Action::create("menu-duck-mask", _("Show/Hide Control Points")) );
...
#define DEFINE_ACTION_SIG(group,x,stock,sig) { Glib::RefPtr<Gtk::Action> action( Gtk::Action::create(x, stock) ); group->add(action,sig); }
	...
	DEFINE_ACTION("mask-widthpoint-position-ducks", _("Show WidthPoints Position Control Points"));
...
*/
	...
	synfigapp::Main::settings().set_value("dock.dialog.1.contents","navigator - info pal_edit pal_browse - tool_options history canvases - layers layergroups");
