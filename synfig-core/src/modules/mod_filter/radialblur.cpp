...
bool
RadialBlur::accelerated_cairorender(Context context, cairo_surface_t *surface,int quality, const RendDesc &renddesc, ProgressCallback *cb)const
{
	// don't do anything at quality 10
	if (quality == 10)
	...
	cairo_surface_t* tmp_surface=cairo_image_surface_create(CAIRO_FORMAT_ARGB32, tmp_surface_width, tmp_surface_height);
	
	// render the layers beneath us
	if(!context.accelerated_cairorender(tmp_surface,quality,desc,cb))
		return false;
	...
	if(!cairo_surface.map_cairo_image())
	{
		synfig::warning("Radial Blur: Cairo map image failed!");
		return false;
	}
	
	CairoSurface cairo_tmp_surface(tmp_surface);
	if(!cairo_tmp_surface.map_cairo_image())
	{
		synfig::warning("Radial Blur: Cairo map image failed!");
		return false;
	}
	//We need to use a accumulator to perform the average sum;
#define TMP_SURFACE(j,i)	(((CairoColorAccumulator)((cairo_tmp_surface)[j][i])))
	...
	for(y=0,pos[1]=tl[1];y<h;y++,pos[1]+=ph)
		for(x=0,pos[0]=tl[0];x<w;x++,pos[0]+=pw)
		{
			...
			{
				pool/=poolsize;
				cairo_surface[y][x]=CairoColor::blend(CairoColor(pool), cairo_surface[y][x], get_amount(), get_blend_method());
			}
		}
	
#undef TMP_SURFACE
	
	cairo_tmp_surface.unmap_cairo_image();
	cairo_surface.unmap_cairo_image();
