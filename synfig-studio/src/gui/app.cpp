...
#undef SET_CANVAS_ACTION
	...
	try {
		App::builder()->add_from_file(ResourceHelper::get_ui_path("studio_menubar.xml"));
	} catch (const Glib::Error& ex) {
		std::cerr << "Building menus failed: " << ex.what();
	}
	try {
		App::builder()->add_from_file(ResourceHelper::get_ui_path("history_toolbar.glade"));
	} catch (const Glib::Error& ex) {
		std::cerr << "Building toolbars failed: " << ex.what();
	}

#if !GTK_CHECK_VERSION(3, 20, 0)
	auto help_object = App::builder()->get_object("help-section-1");
	auto help_menu = Glib::RefPtr<Gio::Menu>::cast_dynamic(help_object);
	help_menu->remove(0);//remove keyobard shortcuts
#endif
#if defined(__APPLE__)
	auto view_object = App::builder()->get_object("view-section-0");
	auto view_menu = Glib::RefPtr<Gio::Menu>::cast_dynamic(view_object);
	view_menu->remove(0);//remove show menubar toggle
#endif
	//add plugins to menu
