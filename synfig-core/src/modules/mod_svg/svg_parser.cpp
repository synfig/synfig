...
Svg_parser::parser_canvas(const xmlpp::Node* node)
{
	if(const xmlpp::Element* nodeElement = dynamic_cast<const xmlpp::Element*>(node)){
		...
		const double view_x = view_box_.get_width() / kux / 2.0;
		const double view_y = view_box_.get_height() / kux / 2.0;
		...
		nodeRoot->set_attribute("view-box", strprintf("%f %f %f %f", -view_x, -view_y, view_x, view_y));
...
Svg_parser::parser_graphics(const xmlpp::Node* node, xmlpp::Element* root, Style style, const SVGMatrix& mtx_parent)
{
	if(const xmlpp::Element* nodeElement = dynamic_cast<const xmlpp::Element*>(node)){
		...
		const std::string label = fetch_element_label_or_id(nodeElement);
...
{
	const Glib::ustring rect_label = fetch_element_label_or_id(nodeElement);
...
{
	const Glib::ustring circle_label = fetch_element_label_or_id(nodeElement);
...
static Glib::ustring
fetch_element_label_or_id(const xmlpp::Element* nodeElement)
{
	if (!nodeElement) {
		synfig::error("SVG Parser: can't fetch SVG node label or id because node is null");
		return "";
	}
