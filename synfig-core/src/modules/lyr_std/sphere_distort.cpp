...
////////
bool
Layer_SphereDistort::accelerated_cairorender(Context context, cairo_surface_t *surface,int quality, const RendDesc &renddesc, ProgressCallback *cb)const
{
	/*	Things to consider:
	 1) Block expansion for distortion (ouch... quality level??)
	 2) Bounding box clipping
	...
	background=cairo_image_surface_create(CAIRO_FORMAT_ARGB32, nw, nh);
	// render the background
	if(!context.accelerated_cairorender(background,quality,r,cb))
	{
	...
	for(y = 0; y < h; ++y, sample[1] += ph)
	{
		...
		for(x = 0; x < w; ++x, sub[0] += pw)
		{
			...
			if(!(xs >= 0 && xs < nw && ys >= 0 && ys < nh))
			{
				csurface[y][x]=context.get_cairocolor(trans).premult_alpha();
				continue;
			}
			
			//sample at that pixel location based on the quality
			if(quality <= 4)	// cubic
				csurface[y][x]=cbackground.cubic_sample_cooked(xs, ys).premult_alpha();
			else if(quality <= 5) // cosine
				csurface[y][x]=cbackground.cosine_sample_cooked(xs, ys).premult_alpha();
			else if(quality <= 6) // linear
				csurface[y][x]=cbackground.linear_sample_cooked(xs, ys).premult_alpha();
			else				// nearest
				csurface[y][x]=cbackground[round_to_int(ys)][round_to_int(xs)].premult_alpha();
		}
	}
