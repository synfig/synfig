...
Renderer::initialize()
{
	...
	// init debug options
	if (const char *s = DEBUG_GETENV("SYNFIG_RENDERING_DEBUG_TASK_LIST_LOG"))
		debug_options.task_list_log = s;
	if (const char *s = DEBUG_GETENV("SYNFIG_RENDERING_DEBUG_TASK_LIST_OPTIMIZED_LOG"))
		debug_options.task_list_optimized_log = s;
	if (const char *s = DEBUG_GETENV("SYNFIG_RENDERING_DEBUG_RESULT_IMAGE"))
