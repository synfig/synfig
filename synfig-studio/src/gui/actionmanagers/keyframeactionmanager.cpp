...
KeyframeActionManager::refresh()
{
	...
	//activate actions depending on context
	{
		...
		const Time begin_time = canvas_interface_->get_canvas()->rend_desc().get_time_start();
		const Time end_time = canvas_interface_->get_canvas()->rend_desc().get_time_end();
...
KeyframeActionManager::get_context_menu_for_keyframe()
{
	if (!menu_keyframe_) {
		// this popup menu is used from Widget_Keyframe_List and Dock_Keyframe
		menu_keyframe_ = Gio::Menu::create();
		...
		menu_keyframe_->append_item(item);
		item = ActionWidgetHelper::create_menu_item_for_action("keyframe.properties");
		menu_keyframe_->append_item(item);
		item = ActionWidgetHelper::create_menu_item_for_action("keyframe.toggle");
		menu_keyframe_->append_item(item);
		item = ActionWidgetHelper::create_menu_item_for_action("keyframe.description-set");
		menu_keyframe_->append_item(item);
	}
