...
utf8_to_utf32(const std::string& text)
{
	...
	FriBidiStrIndex unicode_len = fribidi_charset_to_unicode(
		FRIBIDI_CHAR_SET_UTF8, text.c_str(), text.size(), unicode.data());
...
#if HAVE_HARFBUZZ
static TextLine
reorder_runs_visually(const TextLine& runs, FriBidiParType base_dir)
{
	...
	// direction.
	bool rtl_base =
		(base_dir == FRIBIDI_PAR_RTL || base_dir == FRIBIDI_TYPE_RTL);
	if (!rtl_base)
		return runs;
...
fetch_text_lines(const std::string& text, int direction)
{
...

	{
		...
		// 0. Do some pre-parsing still in UTF-8
		{
			...
			while (pos != std::string::npos)
			{
				...
				else if (/*parsed_text[pos] == '\r' &&*/ pos + 1 !=
							 std::string::npos &&
						 parsed_text[pos + 1] == '\n')
				{
	...
	std::vector<std::vector<uint32_t>> base_lines;
	{
		...
		while (true)
		{
			auto new_it = std::find_first_of(
				it, unicode.end(), line_endings.begin(), line_endings.end());
...
#if HAVE_HARFBUZZ
	std::vector<std::vector<FriBidiLevel>>
		all_levels;							   // logical-order levels, per line
	...
	for (auto& line : base_lines)
	{
		...
		// FriBiDi + HarfBuzz: don't use FriBiDi simple shaper, just get the
		// BiDi info / character reordering
		...
		FriBidiLevel fribidi_result = fribidi_get_par_embedding_levels_ex(
			bidi_types.data(), bracket_types.data(), line_size, &base_dir,
			bidi_levels.data());
#else
		FriBidiLevel fribidi_result = fribidi_get_par_embedding_levels(
			bidi_types.data(), line_size, &base_dir, bidi_levels.data());
#endif
	...
	// 4. Split text into lines (and text spans according to their script if we
	// know them)
#if !HAVE_HARFBUZZ
	...
	for (size_t li = 0; li < base_lines.size(); ++li)
	{
		...
		for (size_t ci = 0; ci < line.size(); ++ci)
		{
			...
			bool same_run =
				!current_line.empty() && parity == current_parity &&
			...
			{
				hb_direction_t dir =
					parity ? HB_DIRECTION_RTL : HB_DIRECTION_LTR;
		...
		new_lines.push_back(
			reorder_runs_visually(current_line, all_base_dirs[li]));
	}
#endif
...
void
convert_outline_to_contours(FT_Outline* outline,
							rendering::Contour::ChunkList& chunks)
{
	...
	outline_funcs.conic_to = [](const FT_Vector* control, const FT_Vector* to,
								void* contour) -> int
	{
		static_cast<rendering::Contour*>(contour)->conic_to(
			Vector(to->x, to->y), Vector(control->x, control->y));
		return 0;
	};
	outline_funcs.cubic_to = [](const FT_Vector* control1,
								const FT_Vector* control2, const FT_Vector* to,
								void* contour) -> int
	{
		static_cast<rendering::Contour*>(contour)->cubic_to(
			Vector(to->x, to->y), Vector(control1->x, control1->y),
...
void
shift_contour_chunks(rendering::Contour::ChunkList& chunks,
					 const Vector& offset)
{
...
shape_text(const std::vector<TextLine>& lines, hb_font_t* font)
{
	...
	std::unique_ptr<hb_buffer_t, decltype(&hb_buffer_destroy)> safe_buf(
		buffer, hb_buffer_destroy);
	...
	for (const TextLine& line : lines)
	{
		...
		for (const TextSpan& span : line)
		{
			...
			hb_buffer_add_utf32(buffer, span.codepoints.data(),
								span.codepoints.size(), 0, -1);
