...
SYNFIG_LAYER_INIT(Layer_Bevel2);
SYNFIG_LAYER_SET_NAME(Layer_Bevel2,"bevel2");
SYNFIG_LAYER_SET_LOCAL_NAME(Layer_Bevel2,N_("Bevel2"));
SYNFIG_LAYER_SET_CATEGORY(Layer_Bevel2,N_("Stylize"));
SYNFIG_LAYER_SET_VERSION(Layer_Bevel2,"0.1");
...
bool
Layer_Bevel2::set_param(const String &param, const ValueBase &value)
{
...
ValueBase
Layer_Bevel2::get_param(const String &param)const
{
...
Color
Layer_Bevel2::get_color(Context context, const Point &pos)const
{
	/*
    Real softness=param_softness.get(Real());
...
/*
RendDesc
Layer_Bevel2::get_sub_renddesc_vfunc(const RendDesc &renddesc) const
{
	Real softness=param_softness.get(Real());
	int type=param_type.get(int());
...
bool
Layer_Bevel2::accelerated_render(Context context,Surface *surface,int quality, const RendDesc &renddesc, ProgressCallback *cb)const
{
	/*
...
Layer::Vocab
Layer_Bevel2::get_param_vocab(void)const
{
...
Rect
Layer_Bevel2::get_full_bounding_rect(Context context)const
{
	Real softness=param_softness.get(Real());
	Real depth=param_depth.get(Real());
...
Layer_Bevel2::draw_sample(rendering::Task::Handle sub_task, Vector offset, Color color )const {
...
Layer_Bevel2::build_composite_fork_task_vfunc(ContextParams /* context_params */, rendering::Task::Handle sub_task)const
{
