...
StateTransform studio::state_transform;
...
class studio::StateTransform_Context : public sigc::trackable
{
	etl::handle<CanvasView> canvas_view_;
...
public:
	...
	StateTransform_Context(CanvasView* canvas_view);
...
};	// END of class StateTransform_Context

/* === M E T H O D S ======================================================= */

StateTransform::StateTransform():
	Smach::state<StateTransform_Context>("transform")
{
	insert(event_def(EVENT_STOP,&StateTransform_Context::event_stop_handler));
	insert(event_def(EVENT_REFRESH,&StateTransform_Context::event_refresh_handler));
	insert(event_def(EVENT_REFRESH_DUCKS,&StateTransform_Context::event_refresh_ducks_handler));
	insert(event_def(EVENT_UNDO,&StateTransform_Context::event_undo_handler));
	insert(event_def(EVENT_REDO,&StateTransform_Context::event_redo_handler));
	insert(event_def(EVENT_WORKAREA_MOUSE_BUTTON_DOWN,&StateTransform_Context::event_mouse_button_down_handler));
	insert(event_def(EVENT_WORKAREA_MULTIPLE_DUCKS_CLICKED,&StateTransform_Context::event_multiple_ducks_clicked_handler));
	insert(event_def(EVENT_REFRESH_TOOL_OPTIONS,&StateTransform_Context::event_refresh_tool_options));
	insert(event_def(EVENT_WORKAREA_LAYER_CLICKED,&StateTransform_Context::event_layer_click));
}
...
void
StateTransform_Context::load_settings()
{
	String value;

	if(settings.get_value("transform.rotate",value) && value=="1")
		set_rotate_flag(true);
	else
		set_rotate_flag(false);

	if(settings.get_value("transform.scale",value) && value=="1")
		set_scale_flag(true);
	else
		set_scale_flag(false);

	if(settings.get_value("transform.constrain",value) && value=="1")
...
void
StateTransform_Context::save_settings()
{
	settings.set_value("transform.rotate",get_rotate_flag()?"1":"0");
	settings.set_value("transform.scale",get_scale_flag()?"1":"0");
	settings.set_value("transform.constrain",get_constrain_flag()?"1":"0");
}

StateTransform_Context::StateTransform_Context(CanvasView* canvas_view):
	...
	checkbutton_constrain(_("Constrain (Shift)"))
{
	...
	checkbutton_rotate.signal_toggled().connect(sigc::mem_fun(*this,&StateTransform_Context::refresh_rotate_flag));
	checkbutton_scale.signal_toggled().connect(sigc::mem_fun(*this,&StateTransform_Context::refresh_scale_flag));
	checkbutton_constrain.signal_toggled().connect(sigc::mem_fun(*this,&StateTransform_Context::refresh_constrain_flag));
	...
	keypress_connect=get_work_area()->signal_key_press_event().connect(sigc::mem_fun(*this,&StateTransform_Context::key_pressed),false);
	keyrelease_connect=get_work_area()->signal_key_release_event().connect(sigc::mem_fun(*this,&StateTransform_Context::key_released),false);
...
bool
StateTransform_Context::key_pressed(GdkEventKey *event)
{
	switch(event->keyval)
	{
		case GDK_Control_L:
...
bool
StateTransform_Context::key_released(GdkEventKey *event)
{
	switch(event->keyval)
	{
		case GDK_Control_L:
...
void
StateTransform_Context::refresh_tool_options()
{
	App::dialog_tool_options->clear();
	App::dialog_tool_options->set_widget(options_table);
	App::dialog_tool_options->set_local_name(_("Transform Tool"));
	App::dialog_tool_options->set_name("transform");
}



StateTransform_Context::~StateTransform_Context()
{
	save_settings();
...
Smach::event_result
StateTransform_Context::event_refresh_tool_options(const Smach::event& /*x*/)
{
	refresh_tool_options();
...
Smach::event_result
StateTransform_Context::event_stop_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received Stop Event");
	canvas_view_->stop();
	return Smach::RESULT_ACCEPT;
}

Smach::event_result
StateTransform_Context::event_refresh_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received Refresh Event");
...
Smach::event_result
StateTransform_Context::event_refresh_ducks_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received Refresh Ducks");
	canvas_view_->queue_rebuild_ducks();
	return Smach::RESULT_ACCEPT;
}

Smach::event_result
StateTransform_Context::event_undo_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received Undo Event");
	canvas_view_->get_instance()->undo();
	return Smach::RESULT_ACCEPT;
}

Smach::event_result
StateTransform_Context::event_redo_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received Redo Event");
	canvas_view_->get_instance()->redo();
	return Smach::RESULT_ACCEPT;
}

Smach::event_result
StateTransform_Context::event_mouse_button_down_handler(const Smach::event& x)
{
	// synfig::info("STATE TRANSFORM: Received mouse button down Event");
...
Smach::event_result
StateTransform_Context::event_layer_click(const Smach::event& x)
{
	const EventLayerClick& event(*reinterpret_cast<const EventLayerClick*>(&x));

	if(event.layer)
	{
		// synfig::info("STATE TRANSFORM: Received layer click Event, \"%s\"",event.layer->get_name().c_str());
	}
	else
	{
		// synfig::info("STATE TRANSFORM: Received layer click Event with an empty layer.");
	}
...
/*
void
StateTransform_Context::edit_several_waypoints(std::list<synfigapp::ValueDesc> value_desc_list)
{
	Gtk::Dialog dialog(
...
Smach::event_result
StateTransform_Context::event_multiple_ducks_clicked_handler(const Smach::event& /*x*/)
{
	// synfig::info("STATE TRANSFORM: Received multiple duck click event");
	...
	menu->items().push_back(Gtk::Menu_Helpers::MenuElem(_("Edit Waypoints"),
		sigc::bind(
			sigc::mem_fun(
				*this,
				&studio::StateTransform_Context::edit_several_waypoints
			),
			value_desc_list
		)
	));
