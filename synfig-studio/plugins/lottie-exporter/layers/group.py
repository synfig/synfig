...
def change_opacity_group(layer, lottie):
    ...
    for c_layer in reversed(canvas.get_layer_list()):
        ...
        deactivated_time = sorted(flip_time(active_time))
        ...
        animation = gen_hold_waypoints(deactivated_time, c_layer, anim_type)
...
def gen_hold_waypoints(deactivated_time, layer, anim_type):
    """
    Will only be used to modify opacity waypoints, and set zero values where the
    layer is deactivated

    Args:
        deactivated_time (set) : Range of time when the layer will be deactivated
        layer (common.Layer.Layer) : Synfig format layer
