...
Context::hit_check(const Point &pos)const
{
	...
	while(!context->empty() && context.in_z_range())
	{
		// If this layer is active, then go
		// ahead and break out of the loop
		if (context.active() && (context.get_params().ignore_hit_locked || !(*context)->is_hit_locked()))
			break;
...
Context::accelerated_render(Surface *surface,int quality, const RendDesc &renddesc, ProgressCallback *cb) const
{
	...
	const Matrix &transformation_matrix(renddesc.get_transformation_matrix());
	...
	for(;!(context)->empty();++context)
	{
		...
		if(!context.active())
			continue;
		const Rect layer_bounds(Transformation::transform_bounds(transformation_matrix, (*context)->get_bounding_rect()));
