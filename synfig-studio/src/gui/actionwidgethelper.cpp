...
static std::string
reparse_accelerator_label(const std::string& original_accel)
{
	guint accel_key;
	GdkModifierType accel_mods;
	gtk_accelerator_parse(original_accel.c_str(), &accel_key, &accel_mods);
	gchar* accel_label_c = gtk_accelerator_get_label(accel_key, accel_mods);
	std::string accel_label(accel_label_c);
	g_free(accel_label_c);
	return accel_label;
}

bool
ActionWidgetHelper::create_tooltip_for_action(const Glib::RefPtr<Gtk::Tooltip>& tooltip_, const std::string& action_name, const std::string& tooltip_text)
{
	auto accels = App::instance()->get_accels_for_action(action_name);
	std::string accels_txt;
	for (const auto& accel : accels) {
		accels_txt += _("\nAccel: ") + reparse_accelerator_label(accel);
	}
	tooltip_->set_text(tooltip_text + accels_txt);
	return true;
}
...
{
	try {
		const ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		return create_menu_item_for_action(action_name, action_entry.icon_, action_entry.get_label());
	} catch (...) {
...
ActionWidgetHelper::create_menu_item_for_synfigapp_action(const std::string& group_prefix, const std::string& action_name, const std::string& icon_name)
{
	...
	std::string action_icon_name = icon_name;
	if (action_icon_name == ">default<")
		action_icon_name = studio::get_action_icon_name(action_it->second);
	if (!action_icon_name.empty() && action_icon_name != "image-missing")
		item->set_icon(Gio::ThemedIcon::create(action_icon_name + symbolic_suffix));
	return item;
}

Gtk::Button*
ActionWidgetHelper::create_action_button(const std::string& action_name)
{
	try {
		ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		return create_action_button(action_name, action_entry.icon_, "", action_entry.get_tooltip(App::instance()));
	} catch (...) {
		synfig::warning(_("Couldn't find action: %s"), action_name.c_str());
	}
...
Gtk::Button*
ActionWidgetHelper::create_action_button(const std::string& action_name, const std::string& icon_name, const std::string& label, const std::string& tooltip)
{
	Gtk::Button* button = Gtk::manage(new Gtk::Button());
	if (button)
		ActionWidgetHelper::init_button(*button, action_name, icon_name, label, tooltip);
	return button;
}
...
void
ActionWidgetHelper::init_toolbutton(Gtk::ToolButton& button, const std::string& action_name)
{
	try {
		const ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		init_toolbutton(button, action_name, action_entry.icon_, action_entry.get_label(), action_entry.get_tooltip(App::instance()));
	} catch (...) {
		synfig::warning(_("Couldn't find action: %s"), action_name.c_str());
	}
}
...
void
ActionWidgetHelper::init_icon_only_toolbutton(Gtk::ToolButton& button, const std::string& action_name)
{
	try {
		const ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		init_toolbutton(button, action_name, action_entry.icon_, {}, action_entry.get_tooltip(App::instance()));
	} catch (...) {
		synfig::warning(_("Couldn't find action: %s"), action_name.c_str());
	}
}
...
ActionWidgetHelper::init_toolbutton(Gtk::ToolButton& button, const std::string& action_name, const std::string& icon_name, const std::string& label, const std::string& tooltip)
{
	...
	set_widget_tooltip(button, action_name, tooltip);
	button.show();
...
ActionWidgetHelper::init_button(Gtk::Button& button, const std::string& action_name)
{
	try {
		const ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		init_button(button, action_name, action_entry.icon_, action_entry.get_label(), action_entry.get_tooltip(App::instance()));
	} catch (...) {
		synfig::warning(_("Couldn't find action: %s"), action_name.c_str());
	}
}
...
ActionWidgetHelper::init_label_only_button(Gtk::Button& button, const std::string& action_name)
{
	try {
		const ActionDatabase::Entry action_entry = App::get_action_database()->get(action_name);
		init_button(button, action_name, {}, action_entry.get_label(), action_entry.get_tooltip(App::instance()));
	} catch (...) {
		synfig::warning(_("Couldn't find action: %s"), action_name.c_str());
	}
}
