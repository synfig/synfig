...
{
	Canvas::LooseHandle layer_canvas = layer->get_canvas();
	bool is_layer_removed = !layer_canvas;
	if (layer_canvas)
		is_layer_removed = std::find(layer_canvas->begin(), layer_canvas->end(), layer) == layer_canvas->end();

	return is_layer_removed;
}
