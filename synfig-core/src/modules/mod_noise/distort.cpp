...
public:
	...
	{
		Vector offset(std::fabs(displacement[0]) * 0.5, std::fabs(displacement[1]) * 0.5);
...
NoiseDistort::build_composite_fork_task_vfunc(ContextParams /* context_params */, rendering::Task::Handle sub_task) const
{
	...
	task_noise_distort->displacement = param_displacement.get(Vector());
	task_noise_distort->size = param_size.get(Vector());
	task_noise_distort->random.set_seed(param_random.get(int()));
	task_noise_distort->smooth = RandomNoise::SmoothType(param_smooth.get(int()));
	task_noise_distort->detail = param_detail.get(int());
	task_noise_distort->speed = param_speed.get(Real());
	task_noise_distort->turbulent = param_turbulent.get(bool());
	task_noise_distort->time_mark = get_time_mark();

	task_noise_distort->sub_task() = sub_task;
