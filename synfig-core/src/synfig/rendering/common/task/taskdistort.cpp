...
rendering::TaskDistort::set_coords_sub_tasks()
{
	...
	// Add 2 pixels due to possible cubic or cosine sampling
	constexpr int padding = 2;
	required_source_rect.expand_x(padding * upp[0]).expand_y(padding * upp[1]);

	const Vector new_target_size = required_source_rect.get_size().multiply_coords(ppu);
