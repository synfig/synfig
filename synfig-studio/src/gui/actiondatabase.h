...
#ifndef SYNFIG_STUDIO_ACTIONDATABASE_H
#define SYNFIG_STUDIO_ACTIONDATABASE_H
...
public:
	...
	void merge(const ActionDatabase& adb);

	std::vector<std::string> get_groups() const;
	EntryList get_entries() const;
	EntryList get_entries_for_group(const std::string& group) const;

	sigc::signal<void, const Entry&> signal_new_entry_added;
	sigc::signal<void, const Entry&> signal_entry_changed;

private:
	std::map<std::string, Entry> book_;
};

class UserAcceleratorList
{
public:
	/** Maps detailed action name -> accelerator */
	typedef std::map<std::string, Glib::ustring> AcceleratorList;
	...
	void apply(Glib::RefPtr<Gtk::Application> app, const ActionDatabase& action_db) const;

	static AcceleratorList get_custom_accels_only(const Glib::RefPtr<Gtk::Application> app, const ActionDatabase& action_db);

	AcceleratorList accels;
};
...
#endif // SYNFIG_STUDIO_ACTIONDATABASE_H
