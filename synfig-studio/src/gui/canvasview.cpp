...
CanvasView::create_top_toolbar()
{
	...
	{ // Rendering mode ComboBox
		...
		if(container)
		{
			...
			g_warning("Could not get combobox container!");
		}
	}
...
CanvasView::init_menus()
{
	...
	*/
	...
	//Todo what is the "canvas-zoom-fit-2" for? a second zoom-fit?
	action_group->add(
...

	{
		...
		//TODO: what is the second zoom-in for?
		...
		//What is the "canvas-zoom-out-2" for?
...
void
CanvasView::on_set_low_res_pixel_size(int size)
{
	if(auto s_action = App::instance()->lookup_action("lowres-pixel") ){
		s_action->change_state(size);
		work_area->set_low_res_pixel_size(size);
	}
}
...
#define TOGGLE_DUCKS(type, TYPE){ \
			if ( auto s_action = App::instance()->lookup_action("mask-"#type"-ducks") ) \
...
CanvasView::mask_bone_ducks()
{
	...
	if (recursive)
	{
		//WHAT DOES mask_bone_setup_ducks do?
