...
{
	before_options=manage(new Widget_InterpolationEnum(Widget_InterpolationEnum::SIDE_BEFORE));
	before_options->show();
	before_options->set_active(0);

	after_options=manage(new Widget_InterpolationEnum(Widget_InterpolationEnum::SIDE_AFTER));
	after_options->show();
