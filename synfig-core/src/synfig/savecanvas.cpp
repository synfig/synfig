...
xmlpp::Element* encode_segment(xmlpp::Element* root,Segment seg)
{
	...
	encode_vector(root->add_child_element("p1")->add_child_element("vector"),seg.p1);
	encode_vector(root->add_child_element("t1")->add_child_element("vector"),seg.t1);
	encode_vector(root->add_child_element("p2")->add_child_element("vector"),seg.p2);
	encode_vector(root->add_child_element("t2")->add_child_element("vector"),seg.t2);
	return root;
}
...
xmlpp::Element* encode_bline_point(xmlpp::Element* root,BLinePoint bline_point)
{
	...
	encode_vector(root->add_child_element("vertex")->add_child_element("vector"),bline_point.get_vertex());
	encode_vector(root->add_child_element("t1")->add_child_element("vector"),bline_point.get_tangent1());

	if(bline_point.get_split_tangent_both())
		encode_vector(root->add_child_element("t2")->add_child_element("vector"),bline_point.get_tangent2());

	encode_real(root->add_child_element("width")->add_child_element("real"),bline_point.get_width());
	encode_real(root->add_child_element("origin")->add_child_element("real"),bline_point.get_origin());
	return root;
}
...
xmlpp::Element* encode_width_point(xmlpp::Element* root,WidthPoint width_point)
{
	...
	encode_real(root->add_child_element("position")->add_child_element("real"),width_point.get_position());
	encode_real(root->add_child_element("width")->add_child_element("real"),width_point.get_width());
	encode_integer(root->add_child_element("side_before")->add_child_element("integer"),width_point.get_side_type_before());
	encode_integer(root->add_child_element("side_after")->add_child_element("integer"),width_point.get_side_type_after());
	return root;
}
...
xmlpp::Element* encode_dash_item(xmlpp::Element* root, DashItem dash_item)
{
	...
	encode_real(root->add_child_element("offset")->add_child_element("real"),dash_item.get_offset());
	encode_real(root->add_child_element("length")->add_child_element("real"),dash_item.get_length());
	encode_integer(root->add_child_element("side_before")->add_child_element("integer"),dash_item.get_side_type_before());
	encode_integer(root->add_child_element("side_after")->add_child_element("integer"),dash_item.get_side_type_after());
	return root;
}
...
xmlpp::Element* encode_gradient(xmlpp::Element* root,Gradient x)
{
	...
	{
		xmlpp::Element *cpoint(encode_color(root->add_child_element("color"),iter->color));
...
xmlpp::Element* encode_list(xmlpp::Element* root,std::vector<ValueBase> list, Canvas::ConstHandle canvas=0)
{
	...
	while(!list.empty())
	{
		encode_value(root->add_child_element("value"),list.front(),canvas);
...
xmlpp::Element* encode_transformation(xmlpp::Element* root,const Transformation &transformation)
{
	...
	encode_vector(root->add_child_element("offset")->add_child_element("vector"),transformation.offset);
	encode_angle(root->add_child_element("angle")->add_child_element("angle"),transformation.angle);
	encode_angle(root->add_child_element("skew_angle")->add_child_element("angle"),transformation.skew_angle);
	encode_vector(root->add_child_element("scale")->add_child_element("vector"),transformation.scale);
	return root;
}
...
xmlpp::Element* encode_weighted_value(xmlpp::Element* root,types_namespace::TypeWeightedValueBase &type, const ValueBase &data,Canvas::ConstHandle canvas)
{
	...
	encode_real(root->add_child_element("weight")->add_child_element("real"), type.extract_weight(data));
	encode_value(root->add_child_element("value")->add_child_element("value"), type.extract_value(data), canvas);
	return root;
}
...
{
	root->set_name(type.description.name);
	encode_value(root->add_child_element("first")->add_child_element("value"), type.extract_first(data), canvas);
	encode_value(root->add_child_element("second")->add_child_element("value"), type.extract_second(data), canvas);
	return root;
}
...
xmlpp::Element* encode_animated(xmlpp::Element* root,ValueNode_Animated::ConstHandle value_node,Canvas::ConstHandle canvas=0)
{
	...
	{
		xmlpp::Element *waypoint_node=root->add_child_element("waypoint");
		...
		if(iter->get_value_node()->is_exported())
			...
			if(ValueNode_Const::ConstHandle::cast_dynamic(value_node))
			{
				...
				if (data.get_type() == type_canvas)
					...
					encode_value_node(waypoint_node->add_child_element("value_node"),iter->get_value_node(),canvas);
			}
			else
				encode_value_node(waypoint_node->add_child_element("value_node"),iter->get_value_node(),canvas);
		}
...
xmlpp::Element* encode_subtract(xmlpp::Element* root,ValueNode_Subtract::ConstHandle value_node,Canvas::ConstHandle canvas=0)
{
	...
	if(!scalar->get_id().empty())
		...
		encode_value_node(root->add_child_element("scalar")->add_child_element("value_node"),scalar,canvas);

	if(!lhs->get_id().empty())
		root->set_attribute("lhs",lhs->get_relative_id(canvas));
	else
		encode_value_node(root->add_child_element("lhs")->add_child_element("value_node"),lhs,canvas);

	if(!rhs->get_id().empty())
		root->set_attribute("rhs",rhs->get_relative_id(canvas));
	else
		encode_value_node(root->add_child_element("rhs")->add_child_element("value_node"),rhs,canvas);
...
xmlpp::Element* encode_static_list(xmlpp::Element* root,ValueNode_StaticList::ConstHandle value_node,Canvas::ConstHandle canvas=0)
{
	...
	{
		xmlpp::Element	*entry_node=root->add_child_element("entry");
		assert(*iter);
		...
		else
		{
			...
			encode_value_node(entry_node->add_child_element("value_node"),*iter,canvas);
		}
	}
...
xmlpp::Element* encode_dynamic_list(xmlpp::Element* root,ValueNode_DynamicList::ConstHandle value_node,Canvas::ConstHandle canvas=0)
{
	...
	for(iter=value_node->list.begin();iter!=value_node->list.end();++iter)
	{
		...
		if(!iter->value_node->get_id().empty())
			...
			encode_value_node(entry_node->add_child_element("value_node"),iter->value_node,canvas);
...
xmlpp::Element* encode_linkable_value_node(xmlpp::Element* root,LinkableValueNode::ConstHandle value_node,Canvas::ConstHandle canvas=0)
{
	...
	for(i=0;i<value_node->link_count();i++, iter++)
	{
		...
		else if(iter->get_critical())
		{
			...
			encode_value_node(root->add_child_element(value_node->link_name(i))->add_child_element("value_node"),link,canvas);
		}
	}
...
xmlpp::Element* encode_layer(xmlpp::Element* root,Layer::ConstHandle layer)
{
	...
	for(iter=vocab.begin();iter!=vocab.end();++iter)
	{
		...
		{
			xmlpp::Element *node=root->add_child_element("param");
			...
			if(value_node->get_id().empty())
			{
				encode_value_node(node->add_child_element("value_node"),value_node,layer->get_canvas().constant());
			}
			else
			{
		...
		if(iter->get_critical())
		{
			...
			encode_value(node->add_child_element("value"),value,layer->get_canvas().constant());
		}
	}
...
xmlpp::Element* encode_canvas(xmlpp::Element* root,Canvas::ConstHandle canvas)
{
	...
	if(!canvas->is_inline())
	{
		...
		{
			xmlpp::Element* meta_element(root->add_child_element("meta"));
		...
		for(KeyframeList::const_iterator iter=canvas->keyframe_list().begin();iter!=canvas->keyframe_list().end();++iter)
			encode_keyframe(root->add_child_element("keyframe"),*iter,canvas->rend_desc().get_frame_rate());
	}

	// Output the <bones> section
	if((!canvas->is_inline() && !ValueNode_Bone::get_bone_map(canvas).empty()))
	{
		xmlpp::Element *node=root->add_child_element("bones");

		encode_value_node_bone(node->add_child_element("value_node"),ValueNode_Bone::get_root_bone(),canvas);
		...
		for(ValueNode_Bone::BoneList::iterator iter=bone_list.begin();iter!=bone_list.end();++iter)
		{
			...
			encode_value_node_bone(node->add_child_element("value_node"),bone,canvas);
		}
	}
	...
	{
		xmlpp::Element *node=root->add_child_element("defs");
		...
		for(ValueNodeList::const_iterator iter=value_node_list.begin();iter!=value_node_list.end();++iter)
		{
			...
			if(handle<ValueNode_Const>::cast_dynamic(*iter))
			{
				...
				reinterpret_cast<xmlpp::Element*>(encode_value(node->add_child_element("value"),value_node->get_value(),canvas))->set_attribute("id",value_node->get_id());
				continue;
			}
			encode_value_node(node->add_child_element("value_node"),*iter,canvas);
			// writeme
		}

		for(Canvas::Children::const_iterator iter=canvas->children().begin();iter!=canvas->children().end();++iter)
		{
			encode_canvas(node->add_child_element("canvas"),*iter);
		}
	}
	...
	for(iter=canvas->rbegin();iter!=canvas->rend();++iter)
		encode_layer(root->add_child_element("layer"),*iter);
