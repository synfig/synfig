	...
	if(x.key==EVENT_WORKAREA_BOX)
	{
		...
		if(event.button==BUTTON_LEFT)
		{
			...
			if(event.modifier & Gdk::CONTROL_MASK) //zoom out...
			{
				...
				else zoom = std::max(abs(box_span[0]/window_span[0]), abs(box_span[1]/window_span[1]));
			...
			else				// zoom in
			{
				...
				else zoom = std::min(abs(window_span[0]/box_span[0]), abs(window_span[1]/box_span[1]));
