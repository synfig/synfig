...
pushd ETL
./bootstrap.sh && ./configure --enable-optimization=0 && make -j2 --silent && make install
popd
...
pushd synfig-studio
./bootstrap.sh && ./configure --enable-optimization=0 --without-images && make -j2 --silent && make install
popd
