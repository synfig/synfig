...
static void
runge_kutta_cash_karp(Function oscillator, Real& t, MathVector& x, Real& step_size, Real tolerance = 1e-5)
{
	const auto& num_dim = x.size();
	MathVector xf, dy4, dy5;
	...
	while (error > tolerance) {
		...
		error = 1e-2 * tolerance + max_delta(dy4, dy5);
		step_size = 0.9 * step_size * std::pow(tolerance * step_size / error, 1/4.);
		xf = x + dy5;
	}
	x = xf;
	t = tf;
