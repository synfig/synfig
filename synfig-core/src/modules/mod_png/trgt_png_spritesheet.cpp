...
png_trgt_spritesheet::start_frame(synfig::ProgressCallback *callback)
{
	...
	if (get_alpha_mode()==TARGET_ALPHA_MODE_KEEP)
		...
		png_set_IHDR(png_ptr,info_ptr,desc.get_w(),desc.get_h() * numimages,8,PNG_COLOR_TYPE_RGB,PNG_INTERLACE_NONE,PNG_COMPRESSION_TYPE_DEFAULT,PNG_FILTER_TYPE_DEFAULT);
