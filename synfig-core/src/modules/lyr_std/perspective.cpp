	...
	Real c)
{
	...
	if (approximate_not_zero(a))
	{
		...
		if (approximate_greater_or_equal(x0, bounds.minx) && approximate_less_or_equal(x0, bounds.maxx))
		{
			if (out_points)
				out_points[count] = Point(x0, bounds.miny);
			if (count++)
				return true;
		}

		const Real x1 = -(c + bounds.maxy * b) / a;
		if (approximate_greater_or_equal(x1, bounds.minx) && approximate_less_or_equal(x1, bounds.maxx))
		{
			if (out_points)
				out_points[count] = Point(x1, bounds.maxy);
			if (count++)
				return true;
		}
	}
	...
	if (approximate_not_zero(b))
	{
		...
		if (approximate_greater_or_equal(y0, bounds.miny) && approximate_less_or_equal(y0, bounds.maxy))
		{
			if (out_points)
				out_points[count] = Point(bounds.minx, y0);
			if (count++)
				return true;
		}

		const Real y1 = -(c + bounds.maxx * a) / b;
		if (approximate_greater_or_equal(y1, bounds.miny) && approximate_less_or_equal(y1, bounds.maxy))
		{
			if (out_points)
				out_points[count] = Point(bounds.maxx, y1);
			if (count++)
				return true;
		}
	}
...
public:
	explicit OptimalResolutionSolver(const Matrix &matrix) : affine(), len()
	{
		this->matrix = matrix;
...
public:
	...
	public:
		...
		explicit Layer(
			...
			const Matrix &alpha_matrix = Matrix()) : orig_rect(orig_rect),
													 bounds(bounds),
...
protected:
	...
	{
		return (bool)dynamic_cast<const TransformationPerspective *>(&other) || (bool)dynamic_cast<const rendering::TransformationAffine *>(&other);
	}
	...
	virtual void merge_outer_vfunc(const Transformation &other)
	{
		...
		else if (const rendering::TransformationAffine *affine = dynamic_cast<const rendering::TransformationAffine *>(&other))
		{
...
public:
		...
		const Bounds &bounds)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			if (w > horizonw)
			{
				...
				if (found)
					out_bounds.rect.expand(p);
				else
					out_bounds.rect.set_point(p);
				found = true;
				if (w < minw)
					minw = w;
				if (w > maxw)
		...
		Real step)
	{
		...
		for (int i = minlog; i <= maxlog; ++i, w *= step)
		{
			...
			for (int j = 0; j < 4; ++j)
			{
				if (transformed_corners[j][2] && approximate_greater_or_equal(transformed_corners[j][2], w0) && approximate_less_or_equal(transformed_corners[j][2], w1))
				{
				...
				for (int j = 0; j < 2; ++j, ++corners_count)
					layer_corners[corners_count] =
						(norm_matrix * Vector3(
										   ...
										   layer_corners_orig[corners_count][1], 1))
							.to_2d() *
						w0;
			}
...
public:
	...
	virtual void set_coords_sub_tasks()
	{
		...
		if (task && is_valid_coords() && approximate_greater(supersample[0], Real(0)) && approximate_greater(supersample[1], Real(0)))
		{
...
public:
	virtual bool run(RunParams &) const
	{
		...
		const Matrix from_pixels_matrix =
			Matrix3().set_translate(source_rect.get_min()) * Matrix3().set_scale(get_units_per_pixel()) * Matrix3().set_translate(-target_rect.minx, -target_rect.miny);
		...
		const Matrix base_matrix =
			back_matrix * from_pixels_matrix;
		...
		for (List::const_iterator i = sub_tasks.begin(); i != sub_tasks.end() && li != layers.end(); ++i, ++li)
		{
			...
			// matrix
			const Matrix matrix =
				Matrix3().set_translate((*i)->target_rect.minx, (*i)->target_rect.miny) * Matrix3().set_scale((*i)->get_pixels_per_unit()) * Matrix3().set_translate(-(*i)->source_rect.get_min()) * base_matrix;
			const Matrix alpha_matrix =
				li->alpha_matrix * from_pixels_matrix;
			...
			const RectInt rect = RectInt(
									 ...
									 (int)approximate_ceil(rect_float.maxy)) &
								 base_rect;
			if (!rect.is_valid())
				continue;
...
rendering::Task::Token TaskTransformationPerspectiveSW::token(
	DescReal<TaskTransformationPerspectiveSW,
			 TaskTransformationPerspective>("TaskTransformationPerspectiveSW"));
} // namespace
...
public:
	Perspective_Trans(const Perspective *x) : Transform(x->get_guid()), layer(x) {}
...
Perspective::Perspective() : param_src_tl(Point(-2, 2)),
...
void Perspective::sync()
{
	...
	{
		matrix = make_matrix(dest_tl, dest_tr, dest_bl, dest_br) * Matrix().set_scale(1 / src_dist[0], 1 / src_dist[1]) * Matrix().set_translate(-src_tl);
...
Point Perspective::transform(const Point &x) const
{
	if (!valid)
		return Vector::nan();
...
Point Perspective::back_transform(const Point &x) const
{
	if (!valid)
		return Vector::nan();
...
Rect Perspective::transform(const Rect &x) const
{
	return valid
			   ? TransformationPerspective::transform_bounds_perspective(
					 matrix, rendering::Transformation::Bounds(x))
					 .rect
			   : Rect();
}

Rect Perspective::back_transform(const Rect &x) const
{
	return valid
			   ? TransformationPerspective::transform_bounds_perspective(
					 back_matrix, rendering::Transformation::Bounds(x))
					 .rect
			   : Rect();
}
...
Perspective::hit_check(Context context, const Point &p) const
{
	...
	if (clip && !clip_rect.is_inside(pp))
		return Layer::Handle();
	return context.hit_check(pp);
}

Color Perspective::get_color(Context context, const Point &p) const
{
	if (!valid)
		return Color::alpha();
	const Point pp = back_transform(p);
...
Rect Perspective::get_bounding_rect() const
{
	return Rect();
}

Rect Perspective::get_full_bounding_rect(Context context) const
{
	if (!valid)
		return Rect();
...
bool Perspective::set_param(const String &param, const ValueBase &value)
{
