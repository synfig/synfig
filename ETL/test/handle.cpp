...
typedef std::shared_ptr<my_other_test_obj> other_obj_handle;
typedef std::list<std::shared_ptr<my_test_obj>> obj_list;
...
int handle_basic_test()
{
    ...
    {
        std::shared_ptr<my_test_obj> obj_handle = make_shared<my_test_obj>(rand());
    }
    ...
    {
        std::map<string,std::shared_ptr<my_test_obj> > my_map;
...
int handle_general_use_test(void)
{
    ...
    for(i=0;i<NUMBER_OF_OBJECTS;i++)
        my_list.push_back( make_shared<my_test_obj>(rand()) );
    ...
    {
        std::shared_ptr<my_test_obj> a=make_shared<my_test_obj>(27), b=make_shared<my_test_obj>(42);
        a.swap(b);
...
int rhandle_general_use_test(void)
{
    ...
    robj_handle obj=	make_shared<my_test_obj>(rand());
    for(i=0;i<NUMBER_OF_OBJECTS;i++)
    ...
    if(obj.use_count()!=NUMBER_OF_OBJECTS*2+1)
    {
        printf("FAILED!\n");
        printf(__FILE__":%d: On copy, handle count=%d, should be %d.\n",__LINE__,obj.use_count(),NUMBER_OF_OBJECTS*2+1);
        return 1;
    }
...
void test_func(std::shared_ptr<my_test_obj> handle)
{
    if(handle) { int i=handle.use_count(); i++; }
}
...
int loose_handle_test(void)
{
    ...
    std::shared_ptr<my_test_obj> obj_handle_loose;
    std::shared_ptr<my_test_obj> obj_handle2;

    {
        std::shared_ptr<my_test_obj> obj_handle = std::make_shared<my_test_obj>(rand());
    ...
    {
        std::shared_ptr<my_test_obj> a(new my_test_obj(27)), b(new my_test_obj(42));
        a.swap(b);
        if (a->my_id != 42 || b->my_id != 27)
        {
            printf("FAILED!\n");
...
int handle_cast_test()
{
    ...
    std::shared_ptr<my_test_obj> obj;
    std::shared_ptr<my_other_test_obj> other_obj;
    std::shared_ptr<my_other_test_obj> loose_obj;

    other_obj = std::make_shared<my_other_test_obj>();
    loose_obj=other_obj;
