...
Gtk::Button* create_action_button(const std::string& action_name);
Gtk::Button* create_action_button(const std::string& action_name, const std::string& icon_name, const std::string& label, const std::string& tooltip);
...
void init_toolbutton(Gtk::ToolButton& button, const std::string& action_name);
void init_icon_only_toolbutton(Gtk::ToolButton& button, const std::string& action_name);
void init_toolbutton(Gtk::ToolButton& button, const std::string& action_name, const std::string& icon_name, const std::string& label, const std::string& tooltip);
...
bool create_tooltip_for_action(const Glib::RefPtr<Gtk::Tooltip>& tooltip, const std::string& action_name, const std::string& tooltip_text);

/**
 * Create a dynamic tooltip that additionally shows the current accelerator associated to the action
 */
template<class T>
void
set_widget_tooltip(T& widget, const std::string& action_name, const std::string& tooltip)
{
	widget.property_has_tooltip() = true;
	widget.signal_query_tooltip().connect(
		sigc::bind(
			sigc::bind(
				sigc::hide<0>(sigc::hide<0>(sigc::hide<0>( // remove the first 3 args
					sigc::ptr_fun(ActionWidgetHelper::create_tooltip_for_action)))),
				tooltip
				),
			action_name
			)
