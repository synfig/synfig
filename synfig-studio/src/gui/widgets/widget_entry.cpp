...
{
	signal_event_after().connect(sigc::mem_fun(*this, &Widget_Entry::after_event));
}
...
void
Widget_Entry::after_event(GdkEvent *event)
{
...
bool
Widget_Entry::on_event(GdkEvent* event)
{
	SYNFIG_EXCEPTION_GUARD_BEGIN()
	if (event->type == GDK_FOCUS_CHANGE && event->focus_change.in)
			first_selection=true;
	return Entry::on_event(event);
